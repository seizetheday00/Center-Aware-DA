...
sys.path.append('.')
import utils
from dalib.adaptation.semantic import SemanticLoss
...
def main(args: argparse.Namespace):
    ...
    if args.log_results:
        wandb.init(project="DA", entity="CADA", name=args.log_name)
    ...
    train_source_dataset, train_target_dataset, val_dataset, test_dataset, num_classes, args.class_names = \
        utils.get_dataset(args.data, args.root, args.source,
                          args.target,  args.download, train_transform, val_transform)
    ...
    semantic_loss = SemanticLoss(num_classes)
    ...
    if args.phase == 'analysis':
        ...
        pool_layer1 = nn.Sequential(
                nn.AdaptiveAvgPool2d(output_size=(1, 768)),
                nn.Flatten()
            )  
        feature_extractor = nn.Sequential(
            classifier.backbone, pool_layer1, classifier.bottleneck).to(device)
        ...
        head = nn.Linear(classifier_feature_dim, num_classes).to(device)
    ...
    if args.phase == 'test':
        acc1, confmat1 = utils.validate(test_loader, classifier, args, device)
        print(acc1)
        return
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_source_iter, train_target_iter, classifier, domain_adv, mcc_loss, semantic_loss, optimizer,
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
          domain_adv: ConditionalDomainAdversarialLoss, mcc, semantic_loss, optimizer: SGD,
    ...
    semantic_loss_value = AverageMeter('semantic loss', ':3.1f')
    ...
    for i in range(args.iters_per_epoch):
        ...
        semantic_loss_value = semantic_loss(f_s, f_t, y_s, y_t)
        transfer_loss = domain_adv(y_s, f_s, y_t, f_t) + mcc(y_t) + semantic_loss_value
        ...
        if args.log_results:
            wandb.log({'iteration': epoch*args.iters_per_epoch + i, 'loss': loss, 'cls_loss': cls_loss,
                       ...
                       'domain_acc': domain_acc, 'mcc_loss': mcc_loss_value, 'semantic_loss': semantic_loss_value})
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Center-Aware Unsupervised Domain Adaptation')
    # dataset parameters
    ...
    parser.add_argument('--download', default='False',
                        help='whether download the dataset')
    ...
    parser.add_argument('--wd', '--weight-decay', default=1e-3, type=float,
                        ...
                        dest='weight_decay')
    parser.add_argument('-j', '--workers', default=8, type=int, metavar='N', #2
